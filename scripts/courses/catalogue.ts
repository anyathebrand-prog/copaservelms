/**
 * The demonstration course catalogue.
 *
 * Content only — no database access. `scripts/seed-courses.ts` publishes it.
 * Split out when the file passed a thousand lines: a runner and a thousand
 * lines of prose in one file means every content edit re-reads the logic.
 *
 * IMPORTANT — this is demonstration material. It is written to be accurate and
 * useful, but no subject-matter expert has reviewed it. A certificate asserts
 * that its holder was taught something correct, and that assertion is only as
 * good as whoever stands behind the material.
 */

export type Lesson = { title: string; minutes: number; body: string };
export type Module = { title: string; description: string; lessons: Lesson[] };
...
export const CORE_COURSES: Course[] = [NDPA, SECURITY, AML];
